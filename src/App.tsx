...
const App: React.FC<IPods> = (props: IPods) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const date = new Date();
      try {
        const podsResponse = await axios.get("http://localhost:5550/cloud/pod/").then((response) => {
      ...
      try {
        const nodesResponse = await axios.get("http://localhost:5550/cloud/node/").then((response) => {
      ...
      try {
        const jobsResponse = await axios.get("http://localhost:5550/cloud/job").then((response) => {
